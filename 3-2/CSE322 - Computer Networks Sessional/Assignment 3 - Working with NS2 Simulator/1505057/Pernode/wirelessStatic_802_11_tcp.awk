...

{
	...
	temp_node=node;
	if((temp_node+0) >= last_node && strEvent != "N")
	{
		last_node = temp_node+0;
	}
...
#			rTotalDelay += rReceivedTime[ idPacket] - rSentTime[ idPacket ];
			...
			temp_throughput = nBytes*8.00/rDelay[idPacket];
			node_throughput[node] += temp_throughput;
...
END{
	...
	{
		if((node_throughput[i]+0.00)>0.00)
		{
			printf("%d: %15.2f\n", i, node_throughput[i]);
		    print (i, node_throughput[i]) >> "gnuTest.dat"
		}
